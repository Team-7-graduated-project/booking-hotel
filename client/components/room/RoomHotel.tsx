...
const RoomHotel = ({ hotel_id }: Props) => {
  const services = [
    "Free toiletries",
    "Bidet",
    "Toilet",
  ...
  const [check_in, setCheckIn] = useState<Date | null>();
  const [check_out, setCheckOut] = useState<Date | null>();
  ...
  const bookingBody: any = {
    hotel_id: hotel_id,
    room_id: roomsReserve[0]?.room_id,
    check_in: check_in,
    check_out: check_out,
    price: price,
  ...
  const booking = async () => {
    if (!check_in || !check_out || !bookingBody.room_id || !price) {
              ...
              >
                ...
                <div className="md:col-span-1 text-sm 2xl:text-base border md:border-l-0 border-blue-500 p-1.5 flex md:justify-center items-center">
                  <div className="flex md:grid md:grid-cols-2 gap-y-1.5">
                    {[...Array(room.max_people)].map((_, index) => (
                      <div
                        key={index}
