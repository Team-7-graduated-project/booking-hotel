...
const Room = () => {
  ...
  const hotel_id = router.query?.id as string;
  const { data: rooms, isLoading } = useGetHotelRoomsQuery(hotel_id);
  ...
  const INITIAL_DATA: IRoom = {
    ...
    updated_at: new Date(), // Set current date or null
  };
  ...
  const handleCreateRoom = async () => {
    try {
      await createRoom({ ...data, hotel_id }).unwrap();
    ...
    >
      <div className="container mx-auto px-4 lg:px-6 pt-5">
        ...
        <div className="flex flex-col">
          ...
          {rooms?.map((room) => (
            <RoomDetail key={room.room_id} {...room} />
          ))}

          {/* Create room */}
          <div>
            <details className="group select-none w-full">
              <summary className="group w-max flex flex-wrap items-center ml-auto px-2.5">
              ...
              <nav aria-label="Users Nav" className="transition-all">
                <div className="grid grid-cols-9 gap-x-2.5 items-center justify-center text-center even:bg-gray-200 p-2.5">
                  ...
                  <div className="w-full">
                    <input
                      ...
                      onChange={(e) =>
                        setData({ ...data, max_people: +e.target.value })
                      }
                    />
                  </div>
                  <div className="w-full">
...
const RoomDetail = (room: IRoom) => {
  ...
  const handleDeleteRoom = async () => {
    try {
      await deleteRoom(room.room_id as string).unwrap();
  ...
  return (
    <div className="grid grid-cols-9 gap-x-2.5 items-center justify-center border-y text-center p-2.5">
      <div className="col-span-2 w-full">
        {isEdit ? (
          <input
            type="text"
      ...
      <div className="w-full">
        {isEdit ? (
          <input
            type="number"
            className="w-full"
            value={dataEdit.max_people}
            onChange={(e) =>
              setDataEdit({ ...dataEdit, max_people: +e.target.value })
            }
          />
        ) : (
      ...
      <div className="w-full">
        {isEdit ? (
          <input
            ...
            onChange={(e) =>
              setDataEdit({ ...dataEdit, quantity: +e.target.value })
            }
          />
        ) : (
